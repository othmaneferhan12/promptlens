...
interface UseTextToVideoReturn {
  result: VideoResult | null;
  isLoading: boolean;
  error: string | null;
  generate: (opts: TextToVideoOptions) => Promise<VideoResult | null>;
  reset: () => void;
}

export function useTextToVideoPrompt(): UseTextToVideoReturn {
  const [result, setResult] = useState<VideoResult | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const generate = useCallback(async (opts: TextToVideoOptions): Promise<VideoResult | null> => {
    setIsLoading(true);
    setError(null);
    setResult(null);
